...
public class EbuyService {
	...
	public String getVersion() {
		return "Welcome to eShop! You are currently using version 0.0.1";
	}
	...
	public String makeOrder(String request) {
		...
		if (cursor.hasNext()) {
			...
			if (product_cursor.hasNext()) {
				int req_quantity=requestObj.getInt("quantity");
				...
				if(quantity>0 && quantity>=req_quantity) {
					...
					b.put("quantity", quantity-req_quantity);
					...
					dbObject.put("emailId",requestObj.getString("emailId"));
					
					dbObject.put("productId",productId);
					
					order.insert(dbObject);
					
					//insert the updated order id
					BasicDBObject oidsearchQuery = new BasicDBObject();
					searchQuery.put("orderSeqNum", orderId);
					bo.put("orderSeqNum", orderId+1);
					seqNumber.update(oidsearchQuery, bo);
					
					return "{\r\n\"outputCode\":200,\r\n\"message\":\"Success\"\r\n}";
				}
				return "{\r\n\"outputCode\":404,\r\n\"message\":\"Sorry, mentioned quantity of products is not available\"\r\n}";
			}
	...
	public String showOrders() {
		...
		DB db = mongo.getDB("Ebuy");
		DBCollection catalogue = db.getCollection("order");
